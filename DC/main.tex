...
\begin{itemize}
    \item \textbf{Eventual}: Writes are executed on the master only, with asynchronous binlog replication to replicas. This provides the lowest latency but allows reading stale data from replicas.
    \item \textbf{Strong}: Writes are executed on the master, then the coordinator waits for Cabinet-selected quorum replicas to catch up via binlog replication before returning success. This ensures durability and read-after-write consistency.
\end{itemize}
