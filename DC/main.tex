...
\IEEEauthorrefmark{1}018423925, keyaba.gohil@sjsu.edu\\
\IEEEauthorrefmark{2}019106750, jayateerth.kamatgi@sjsu.edu\\
\IEEEauthorrefmark{3}019104319, satheeshkumar.geetharavi@sjsu.edu}
}
...
\begin{itemize}
    ...
    \item \textbf{Per-table timestamp tracking} that provides fine-grained visibility into replication lag. Unlike AWS RDS's coarse ``seconds behind master'' metric, our system tracks the last applied timestamp for each table on each replica, enabling precise identification of which tables are behind and by how many operations.
\end{itemize}
...
For reads, the coordinator inspects replica metrics to choose an appropriate target. For strongly consistent reads, it uses the same Cabinet algorithm as writes to select the best replica based on latency and replication lag, ensuring reads go to replicas that participated in the write quorum. For eventual consistency reads, it routes to the lowest-latency healthy replica. When the master fails, the coordinator calls the SEER service, which scores all healthy replicas using latency, lag, uptime, and crash history and selects a new leader. The coordinator then updates its routing state and resumes handling client requests.
...
\begin{itemize}
    \item \textbf{Eventual}: For writes, the coordinator waits only for the master to apply the update before replying. Replication to replicas happens asynchronously. For reads, the coordinator routes to the lowest-latency healthy replica. This minimizes latency but allows bounded staleness.
    \item \textbf{Strong}: For writes, the coordinator uses Cabinet's quorum-based replication, waiting for acknowledgments from a majority of replicas selected based on performance metrics. For reads, it uses the same Cabinet algorithm to select the best replica, ensuring reads go to replicas that participated in the write quorum and have the latest data. This provides strong read-after-write consistency while distributing read load.
\end{itemize}
...
The coordinator's read path combines consistency semantics with performance-driven routing. Critically, we use the same Cabinet algorithm for both writes and strong reads, ensuring consistency through quorum overlap.

\textbf{Eventual Consistency Reads}: The coordinator fetches metrics and routes to the lowest-latency healthy replica, prioritizing performance over freshness.

\textbf{Strong Consistency Reads}: The coordinator calls the Cabinet service (the same endpoint used for writes) to get the best replicas ranked by latency and replication lag. It then reads from the top-ranked replica. This ensures that strong reads go to the same high-quality replicas that received the quorum writes.

In pseudocode, the read routing logic is:
\begin{lstlisting}[caption=Read Routing Based on Consistency Level]
def route_read(query, consistency):
    if consistency == "EVENTUAL":
        # Route to lowest-latency healthy replica
        metrics = get_metrics_from_collector()
        healthy = [r for r in metrics.replicas
                   if r.is_healthy]
        healthy.sort(key=lambda r: r.latency_ms)
        target = healthy[0] if healthy else master
        return execute_query_on(target, query)
    else:  # STRONG
        # Use Cabinet algorithm (same as writes)
        quorum = cabinet.select_quorum("read")
        best_replica = quorum[0]  # Highest ranked
        result = execute_query_on(best_replica, query)
        if result.failed:
            # Fallback to master
            return execute_query_on(master, query)
        return result
\end{lstlisting}

This design has two key benefits:
\begin{enumerate}
    \item \textbf{Consistency through quorum overlap}: Since strong writes wait for Cabinet-selected replicas to catch up, reading from the same replicas guarantees access to the latest committed data.
    \item \textbf{Load distribution}: Instead of always hitting the master for strong reads, load is distributed across the best-performing replicas.
\end{enumerate}

The algorithm is $O(n)$ in the number of replicas. Under Eventual consistency, we prioritize latency over freshness. Under Strong consistency, we leverage Cabinet's scoring to read from replicas that are both fast and up-to-date.
...
\begin{tikzpicture}[scale=0.5, transform shape]
    ...
    \node[font=\footnotesize] at (-2,0.5) {App};
    \draw (-2,2) circle (0.3);
    \draw (-2,1.7) -- (-2,1.2);
...
\begin{itemize}
    ...
    \item \textbf{Read routing}: For eventual consistency, selects the lowest-latency healthy replica. For strong consistency, uses Cabinet algorithm to select the best replica (same logic as quorum writes)
...
\begin{itemize}
    ...
    \item A \texttt{\_metadata} table tracking the global last applied timestamp:
...
CREATE TABLE _metadata (
  ...
  last_update TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    ...
    \end{lstlisting}
    \item A \texttt{\_table\_timestamps} table for per-table replication tracking:
    \begin{lstlisting}[language=sql,basicstyle=\ttfamily\tiny]
CREATE TABLE _table_timestamps (
  table_name VARCHAR(255) PRIMARY KEY,
  last_timestamp BIGINT DEFAULT 0,
  last_update TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
    \end{lstlisting}
    This enables fine-grained visibility into which specific tables are behind on each replica---a significant improvement over AWS RDS's coarse ``seconds behind master'' metric.
...
\caption{Component Diagram showing service dependencies and communication paths.}
\label{fig:component}
\end{figure}
...
\begin{tikzpicture}[scale=0.65, transform shape,
    node distance=0.8cm,
    container/.style={rectangle, draw, fill=blue!10, minimum width=2cm, minimum height=0.7cm, font=\scriptsize},
    db/.style={cylinder, draw, fill=orange!20, shape border rotate=90, aspect=0.25, minimum height=0.7cm, minimum width=1cm, font=\scriptsize},
    host/.style={rectangle, draw, dashed, fill=gray!5, inner sep=0.3cm},
    network/.style={rectangle, draw, fill=green!10, minimum width=8cm, minimum height=0.5cm, font=\scriptsize}]
    
    % Host machine
    \node[host, minimum width=10cm, minimum height=6cm] (host) {};
    \node[above] at (host.north) {\textbf{Host Machine (Docker)}};
    
    % Docker network
    \node[network] (net) at (0,-2.2) {db-network (172.20.0.0/16)};
    
    % Coordinator
    \node[container, fill=red!15] (coord) at (0,1.8) {Coordinator :9000};
    
    % Timestamp services
    \node[container, fill=yellow!20] (ts1) at (-3.5,0.8) {Timestamp-1 :9001};
    \node[container, fill=yellow!20] (ts2) at (-3.5,0) {Timestamp-2 :9002};
    
    % Support services
    \node[container, fill=purple!15] (metrics) at (3.5,0.8) {Metrics :9003};
    \node[container, fill=purple!15] (cabinet) at (3.5,0) {Cabinet :9004};
    \node[container, fill=purple!15] (seer) at (3.5,-0.8) {SEER :9005};
    
    % MySQL instances
    \node[db] (db1) at (-2,-1.2) {MySQL-1 :3306};
    \node[db] (db2) at (-0.5,-1.2) {MySQL-2 :3307};
    \node[db] (db3) at (1,-1.2) {MySQL-3 :3308};
    \node[db] (db4) at (2.5,-1.2) {MySQL-4 :3309};
    
    % Labels
    \node[font=\tiny] at (-2,-0.7) {Master};
    \node[font=\tiny] at (0.5,-0.7) {Replicas};
...
\begin{tikzpicture}[scale=0.75, transform shape,
    state/.style={rectangle, rounded corners, draw, fill=green!10, minimum width=2cm, minimum height=0.8cm, font=\footnotesize},
    initial/.style={circle, draw, fill=black, inner sep=2pt},
    final/.style={circle, draw, double, fill=black!20, inner sep=2pt}]
    
    % States
    \node[initial] (init) at (-4,0) {};
    \node[state] (stopped) at (-2,0) {Stopped};
    \node[state] (starting) at (1,0) {Starting};
    \node[state, fill=blue!15] (healthy) at (4,0) {Healthy};
    \node[state, fill=yellow!20] (lagging) at (4,-2) {Lagging};
    \node[state, fill=red!15] (unhealthy) at (1,-2) {Unhealthy};
    \node[state, fill=orange!20] (promoting) at (-2,-2) {Promoting};
    \node[state, fill=green!25] (master) at (-2,-4) {Master};
    
    % Transitions
    \draw[->, thick] (init) -- (stopped);
    \draw[->, thick] (stopped) -- node[above, font=\tiny] {start} (starting);
    \draw[->, thick] (starting) -- node[above, font=\tiny] {connected} (healthy);
    \draw[->, thick] (healthy) -- node[right, font=\tiny] {lag > threshold} (lagging);
    \draw[->, thick] (lagging) -- node[below, font=\tiny] {caught up} (healthy);
    \draw[->, thick] (lagging) -- node[above, font=\tiny] {timeout} (unhealthy);
    \draw[->, thick] (healthy) -- node[above, font=\tiny] {conn lost} (unhealthy);
    \draw[->, thick] (unhealthy) -- node[above, font=\tiny] {reconnect} (starting);
    \draw[->, thick] (healthy) -- node[left, font=\tiny] {SEER elected} (promoting);
    \draw[->, thick] (promoting) -- node[left, font=\tiny] {promoted} (master);
    \draw[->, thick] (unhealthy) -- node[below, font=\tiny] {stop} (stopped);
    
\end{tikzpicture}
\caption{State Diagram for MySQL Replica Lifecycle showing transitions between operational states.}
\label{fig:state}
\end{figure}
...
\hline
Coordinator & 9000 & /query, /status, /table-timestamps \\
\hline
Timestamp 1 & 9001 & /timestamp, /health, /reset \\
\hline
Timestamp 2 & 9002 & /timestamp, /health, /reset \\
\hline
Metrics & 9003 & /metrics, /table-timestamps \\
\hline
...
\begin{tikzpicture}[scale=0.6, transform shape,
    class/.style={rectangle, draw, fill=blue!10, minimum width=2.5cm, font=\scriptsize},
    classname/.style={rectangle, draw, fill=blue!25, minimum width=2.5cm, font=\scriptsize\bfseries}]
    
    % Coordinator
    \node[classname] (coordname) at (0,3) {Coordinator};
    \node[class, below=0pt of coordname, text width=2.3cm, align=left] (coordattr) {current\_master\\current\_replicas\\state\_lock};
    \node[class, below=0pt of coordattr, text width=2.3cm, align=left] (coordmeth) {execute\_query()\\get\_timestamp()\\handle\_failover()};
    
    % Cabinet
    \node[classname] (cabname) at (-4,0) {CabinetService};
    \node[class, below=0pt of cabname, text width=2.3cm, align=left] (cabattr) {metrics\_url\\quorum\_size};
    \node[class, below=0pt of cabattr, text width=2.3cm, align=left] (cabmeth) {select\_quorum()\\calculate\_weights()};
    
    % SEER
    \node[classname] (seername) at (4,0) {SEERService};
    \node[class, below=0pt of seername, text width=2.3cm, align=left] (seerattr) {metrics\_url\\weights};
    \node[class, below=0pt of seerattr, text width=2.3cm, align=left] (seermeth) {elect\_leader()\\compute\_score()};
    
    % Timestamp
    \node[classname] (tsname) at (-4,3) {TimestampService};
    \node[class, below=0pt of tsname, text width=2.3cm, align=left] (tsattr) {counter\\server\_id};
    \node[class, below=0pt of tsattr, text width=2.3cm, align=left] (tsmeth) {get\_timestamp()\\reset()};
    
    % Metrics
    \node[classname] (metname) at (4,3) {MetricsCollector};
    \node[class, below=0pt of metname, text width=2.3cm, align=left] (metattr) {replica\_metrics\\poll\_interval};
    \node[class, below=0pt of metattr, text width=2.3cm, align=left] (metmeth) {collect\_metrics()\\get\_health()};
    
    % Relationships
    \draw[->, thick] (coordmeth.west) -- ++(-0.5,0) |- (tsmeth.east) node[midway, above, font=\tiny] {};
    \draw[->, thick] (coordmeth.south) -- ++(0,-0.3) -| (cabmeth.north);
    \draw[->, thick] (coordmeth.south) -- ++(0,-0.3) -| (seermeth.north);
    \draw[->, thick] (coordattr.east) -- ++(0.5,0) |- (metmeth.west);
    \draw[dashed, ->] (cabattr.south) -- ++(0,-0.8) -| (metmeth.south) node[near start, right, font=\tiny] {fetches};
    \draw[dashed, ->] (seerattr.south) -- ++(0,-0.5) -| ([xshift=0.3cm]metmeth.south) node[near start, left, font=\tiny] {fetches};
    
\end{tikzpicture}
\caption{Class/Module Diagram showing service modules and their relationships.}
\label{fig:class}
\end{figure}
...
\centering
\begin{tikzpicture}[scale=0.65, transform shape,
    object/.style={rectangle, draw, fill=yellow!15, minimum width=2.8cm, font=\scriptsize},
    objectname/.style={rectangle, draw, fill=yellow!30, minimum width=2.8cm, font=\scriptsize}]
    
    % Coordinator instance
    \node[objectname] (coord) at (0,2.5) {\underline{coord:Coordinator}};
    \node[object, below=0pt of coord, text width=2.6cm, align=left] (coordval) {master=instance-1\\replicas=[2,3,4]\\mode=STRONG};
    
    % Request
    \node[objectname] (req) at (-4,2.5) {\underline{req:QueryRequest}};
    \node[object, below=0pt of req, text width=2.6cm, align=left] (reqval) {query="INSERT..."\\consistency=STRONG\\timestamp=107};
    
    % MySQL Master
    \node[objectname] (db1) at (0,-0.5) {\underline{db1:MySQLInstance}};
    \node[object, below=0pt of db1, text width=2.6cm, align=left] (db1val) {host=instance-1\\role=MASTER\\binlog\_pos=1234};
    
    % Replica
    \node[objectname] (db2) at (4,-0.5) {\underline{db2:MySQLInstance}};
    \node[object, below=0pt of db2, text width=2.6cm, align=left] (db2val) {host=instance-2\\role=REPLICA\\lag=0};
    
    % Quorum result
    \node[objectname] (quorum) at (4,2.5) {\underline{q:QuorumResult}};
    \node[object, below=0pt of quorum, text width=2.6cm, align=left] (quorumval) {replicas=[2,3]\\size=2\\achieved=true};
    
    % Links
    \draw[->, thick] (reqval.east) -- (coordval.west) node[midway, above, font=\tiny] {processes};
    \draw[->, thick] (coordval.south) -- (db1.north) node[midway, right, font=\tiny] {writes to};
    \draw[->, thick] (coordval.east) -- (quorum.west) node[midway, above, font=\tiny] {awaits};
    \draw[dashed, ->] (db1val.east) -- (db2val.west) node[midway, above, font=\tiny] {replicates};
    
\end{tikzpicture}
\caption{Object Diagram showing runtime instances during a write operation with timestamp 107.}
\label{fig:object}
\end{figure}
...
\begin{tikzpicture}[scale=0.6, transform shape,
    ...
    stop/.style={circle, draw, double, fill=black, inner sep=3pt},
    action/.style={rectangle, rounded corners, draw, fill=blue!15, minimum width=2cm, minimum height=0.6cm, font=\scriptsize},
    decision/.style={diamond, draw, fill=yellow!20, minimum width=1cm, minimum height=1cm, font=\scriptsize, inner sep=1pt},
    fork/.style={rectangle, draw, fill=black, minimum width=2cm, minimum height=3pt}]
    
    \node[start] (start) at (0,0) {};
    \node[action] (receive) at (0,-1) {Receive Query};
    \node[decision] (iswrite) at (0,-2.5) {Write?};
    
    % Write path
    \node[action] (getts) at (-2.5,-4) {Get Timestamp};
    \node[action] (writemaster) at (-2.5,-5.2) {Write to Master};
    \node[decision] (strong) at (-2.5,-6.7) {Strong?};
    \node[action] (getquorum) at (-2.5,-8.2) {Get Quorum};
    \node[action] (waitreplica) at (-2.5,-9.4) {Wait Replicas};
    
    % Read path  
    \node[action] (selectreplica) at (2.5,-4) {Select Replica};
    \node[action] (readdata) at (2.5,-5.2) {Read Data};
    
    % Merge
    \node[action] (respond) at (0,-11) {Return Response};
    \node[stop] (stop) at (0,-12.2) {};
    
    % Connections
    \draw[->, thick] (start) -- (receive);
    \draw[->, thick] (receive) -- (iswrite);
    \draw[->, thick] (iswrite) -- node[left, font=\tiny] {yes} (getts);
    \draw[->, thick] (iswrite) -- node[above, font=\tiny] {no} (selectreplica);
    \draw[->, thick] (getts) -- (writemaster);
    \draw[->, thick] (writemaster) -- (strong);
    \draw[->, thick] (strong) -- node[left, font=\tiny] {yes} (getquorum);
    \draw[->, thick] (strong.east) -- ++(1,0) |- (respond.west) node[near start, above, font=\tiny] {no};
    \draw[->, thick] (getquorum) -- (waitreplica);
    \draw[->, thick] (waitreplica) |- (respond);
    \draw[->, thick] (selectreplica) -- (readdata);
    \draw[->, thick] (readdata) |- (respond);
    \draw[->, thick] (respond) -- (stop);
...
\begin{figure}[!t]
\centering
\lstinputlisting[basicstyle=\ttfamily\tiny, frame=single, breaklines=true, columns=fullflexible, keepspaces=true, language={}, linerange={1-50}]{images/logs/scenario1_success.txt}
\caption{Backend logs of successful Strong consistency write operation showing timestamp service, Cabinet quorum selection, and replica catch-up verification.}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item instance-2: latency=0.077, stability=0.973, lag=1.0, \textbf{total=0.620}
        \item instance-3: latency=0.125, stability=0.986, lag=1.0, \textbf{total=0.645}
        \item instance-4: latency=0.143, stability=0.982, lag=1.0, \textbf{total=0.650}
    \end{itemize}
    \item SEER returns the election result:
    \begin{lstlisting}[language=json,basicstyle=\ttfamily\tiny]
{"leader_id": "instance-4", "score": 0.650}
    \end{lstlisting}
...
curl -X POST \
  http://localhost:9000/admin/start-instance \
...
\begin{figure}[!t]
\centering
\lstinputlisting[basicstyle=\ttfamily\tiny, frame=single, breaklines=true, columns=fullflexible, keepspaces=true, language={}, linerange={1-55}]{images/logs/scenario2_master_failure.txt}
\caption{Backend logs of master failure scenario showing SEER election scoring, leader promotion, and replica reconfiguration.}
...
\textbf{Data Consistency}: The data remains consistent because quorum ensures that any subsequent strong read will go to one of the Cabinet-selected replicas that received the write. Since strong reads use the same Cabinet algorithm as writes, they are routed to the highest-quality replicas---the same ones that participated in the write quorum.

\begin{figure}[!t]
\centering
\lstinputlisting[basicstyle=\ttfamily\tiny, frame=single, breaklines=true, columns=fullflexible, keepspaces=true, language={}, linerange={1-55}]{images/logs/scenario3_replica_failure.txt}
\caption{Backend logs of replica failure scenario showing Cabinet excluding unhealthy replicas and quorum still achieved.}
...
\begin{figure}[!t]
\centering
\lstinputlisting[basicstyle=\ttfamily\tiny, frame=single, breaklines=true, columns=fullflexible, keepspaces=true, language={}, linerange={1-55}]{images/logs/scenario4_timestamp_failure.txt}
\caption{Backend logs of timestamp server failure showing automatic failover and continued writes with even timestamps from backup server.}
...
curl -X POST http://localhost:9000/query \
  ...
  -d '{
    ...
    \begin{lstlisting}[language=bash]
for i in 2 3 4; do
  docker exec mysql-instance-$i mysql \
    -u root -prootpass \
...
\textbf{Objective}: Verify reads are routed appropriately based on consistency level---eventual reads go to lowest-latency replica, strong reads use Cabinet algorithm.

\textbf{Preconditions}: Metrics collector has recent data showing instance-4 has lowest latency.

\textbf{Steps}:
\begin{enumerate}
    \item Check metrics to identify replica rankings:
    \begin{lstlisting}[language=bash]
...
#   instance-4: 6ms
    \end{lstlisting}
    \item Execute SELECT query with EVENTUAL consistency:
    \begin{lstlisting}[language=bash]
curl -X POST http://localhost:9000/query \
  -H "Content-Type: application/json" \
  -d '{
    ...
    \end{lstlisting}
    \item Execute SELECT query with STRONG consistency:
    \begin{lstlisting}[language=bash]
curl -X POST http://localhost:9000/query \
  -H "Content-Type: application/json" \
  -d '{
    ...
    \end{lstlisting}
    \item Verify response metadata shows expected replica for each
\end{enumerate}

\textbf{Expected Result}: 
\begin{itemize}
    \item EVENTUAL: Routes to instance-4 (lowest latency)
    \item STRONG: Routes to Cabinet-selected best replica (considers both latency and replication lag)
\end{itemize}

\textbf{Actual Result}: PASS - Eventual read executed on instance-4 (6.38ms latency). Strong read executed on Cabinet-selected replica with best combined score.
...
curl -X POST http://localhost:9000/query \
  ...
  -d '{
    "query": "INSERT INTO users (name, email)
              VALUES ('AfterFailover', 'after@test.com')",
    "consistency": "STRONG"
  }'
    \end{lstlisting}
...
async def insert(client, i):
    ...
    query += f"VALUES ('User{i}', 'user{i}@test.com')"
    resp = await client.post(
        "http://localhost:9000/query",
        json={"query": query, "consistency": "STRONG"}
    )
...
\begin{itemize}
    \item \textbf{Write-heavy (100\% writes)}: 20-30 ops/sec with Strong consistency. The bottleneck is the quorum acknowledgment latency, as each write must wait for 2 of 3 replicas.
...
Table~\ref{tab:latency-percentiles} shows the latency distribution for Strong vs Eventual writes based on our measurements.
...
\subsection{Comparison of Consistency Modes}

We compared our two consistency modes to understand the performance trade-offs:

\begin{itemize}
    \item \textbf{Eventual}: Writes are executed on the master only, with asynchronous binlog replication to replicas. Reads are routed to the lowest-latency healthy replica. This provides the lowest latency but allows reading stale data.
    \item \textbf{Strong}: Writes are executed on the master, then the coordinator waits for Cabinet-selected quorum replicas to catch up via binlog replication before returning success. Reads use the same Cabinet algorithm to select the best replica, ensuring they go to replicas that have the latest data. This ensures durability and read-after-write consistency while distributing read load.
\end{itemize}
...
\hline
Strong (Cabinet quorum) & 35ms & Waits for quorum catch-up \\
\hline
\end{tabular}
\label{tab:comparison}
\end{table}

Strong consistency with Cabinet's adaptive quorum selection provides durability guarantees while maintaining reasonable latency. The Cabinet algorithm consistently selects the fastest replicas for the quorum, avoiding stragglers and reducing tail latency. Eventual consistency provides nearly 2x lower latency by only waiting for master acknowledgment, making it suitable for workloads that can tolerate temporary staleness.
...
\begin{enumerate}
    \item \textbf{Simplicity vs. Completeness}: Starting with a minimal implementation helped us understand core concepts before adding complexity. Implementing MySQL's GTID-based binlog replication required careful attention to server IDs, GTID tracking, and replica configuration. We learned that proper error handling during failover and replica reconfiguration is crucial for maintaining replication consistency across the cluster.
    ...
    \item \textbf{Trade-offs are Real}: The CAP theorem isn't just theoretical-we directly observed the consistency/availability trade-off. When we configured the system for Strong consistency, writes required quorum acknowledgment which increased latency but ensured durability. When we relaxed to Eventual consistency, we could observe faster writes but with the possibility of reading stale data from replicas. Understanding these trade-offs at a visceral level was one of the most valuable outcomes of the project.
